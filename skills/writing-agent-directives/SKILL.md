...
description: Use when writing instructions that guide agent behavior - skills, AGENTS.md files, subagent prompts, system prompts. Covers token efficiency, compliance techniques, and discovery optimization.
user-invocable: false
---

# Writing Agent Directives

**REQUIRED:** Also apply `prompt-security-hardening` when using this skill. Directives that demonstrate unsafe secrets handling teach agents to leak secrets.

## Core Principles

**1. Agents are smart.** Only write what the agent does not already know. Challenge each line: does this justify its token cost?
...
The `description` field determines if an agent finds your skill.
...
**Keywords:** Include error messages, symptoms, and tool names an agent might search for.

## Compliance Techniques

Modern LLM agents are highly responsive to instructions. Lead with context and motivation; reserve imperatives for critical boundaries.
...
Many LLM agents parse XML-style structure effectively. Use it for multi-part directives:
...
LLM agents can overengineer. Include this when needed:
...
| Mistake | Fix |
|---------|-----|
| Verbose explanations | Agents know basics - omit |
