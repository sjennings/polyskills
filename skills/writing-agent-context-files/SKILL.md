...
**REQUIRED BACKGROUND:** Read `writing-agent-directives` for foundational guidance on token efficiency, compliance techniques, and directive structure.

## Core Principle

AGENTS.md files preserve project context so humans do not have to re-explain architectural intent every session.
...
Agents and harnesses can use AGENTS.md files from the current directory up to the repository root:
...
| Boundaries | What agents can/cannot edit |
...
**Do NOT use @ syntax** (e.g., `@./service.ts`). This force-loads files into context, burning tokens. Just name the files; agents can read them when needed.
...
| Using @ to reference files | Just name files, let agents read on demand |
