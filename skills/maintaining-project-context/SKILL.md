...
**REQUIRED SUB-SKILL:** Use `writing-agent-context-files` for all AGENTS.md creation and updates.

## Core Principle

AGENTS.md files document contracts and architectural intent. When code changes contracts, the documentation must update. Stale documentation is worse than no documentation.
...
Before any updates, detect whether the repository already has project context:
...
|-----------------|--------|--------|
| Yes | AGENTS.md-canonical | Read and update AGENTS.md files |
| No | New AGENTS.md-canonical | Create AGENTS.md files where context is needed |

**Key principle:** Use AGENTS.md as the canonical shared project context format. Write new and updated context to AGENTS.md by default.

## When to Update Context Files

| Change Type | Update Required? | What to Update |
|-------------|------------------|----------------|
| New domain/module | Yes | Create domain AGENTS.md |
...
### Step 2: Map Changes to AGENTS.md Files

For each significant change, determine which AGENTS.md file should document it:
...
For each affected AGENTS.md file, verify:
...
### Step 4: Update or Create AGENTS.md Files
...
**For new domains:**
1. Create `<domain>/AGENTS.md` using the template from `writing-agent-context-files`
...
| Putting domain info in root | Use domain AGENTS.md files for domain contracts |
...
**Called by:**
- **project-context-librarian subagent** - Uses this skill to coordinate updates
- Development phase or branch completion workflows

**Uses:**
- **writing-agent-context-files** - For AGENTS.md creation and updates
